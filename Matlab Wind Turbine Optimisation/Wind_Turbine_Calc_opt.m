...
function negcp = Wind_Turbine_Calc_opt(B, vfar, omega, R, C, Beta)
...
%     TurbPerformance.CX = TurbPerformance.CL.*sin(TurbPerformance.theta) - TurbPerformance.CD.*cos(TurbPerformance.theta);
    ...
    negcp = -TurbPerformance.Power/(0.5*1.2*pi*R^2*vfar^3);
%% Plots
figure(1), subplot(2,2,1), [hAx,hLine1,hLine2] = plotyy(r,C,r,rad2deg(Beta)); xlabel('Radius(m)');
...
text(0,max(Torque),['\omega = ', num2str(omega*30/pi), ' rpm'])
subplot(2,2,4), plot(r,a,r,a_tang), xlabel('Radius(m)'), ylabel('Induction Factor')
legend('Axial', 'Tangential'), pause(0.001)
