...
function TurbPerformance = Wind_Turbine_Calc(B, vfar, omega, R, C, Beta)
% Vfar = farfield wind speed in m/s
% omega = wind turbine rotational speed in Rad/s
% R = Blade swept radius in m
% C = Vector containg chord lengths in m at evenly spaced points along blade 
% B = Vector containg twist angles in radians at evenly spaced points along
...
while delta > 0.0001;
    ...
    [CL, CD] = Turb_Aero_Coeff(aoa,Re,r);
...
%% Plots
figure(3), subplot(2,2,1), [hAx,hLine1,hLine2] = plotyy(r,C,r,rad2deg(Beta)); xlabel('Radius(m)');
ylabel(hAx(1),'Chord Length (m)');  ylabel(hAx(2),'Twist angle (deg)');
subplot(2,2,2), plot(r,rad2deg(aoa)), xlabel('Radius(m)'), ylabel('\alpha (deg)')
subplot(2,2,3), plot(r,Torque), xlabel('Radius(m)'), ylabel('Torque (N)')
subplot(2,2,4), plot(r,a,r,a_tang), xlabel('Radius(m)'), ylabel('Induction Factor')
